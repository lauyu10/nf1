	...
	<body>
		...
		<main role="main">
			...
			<section class="jumbotron text-center">
				<div class="container">
					<h1 class="jumbotron-heading">Results for season <?php echo($_SESSION['year_season'] . " of " . $_SESSION['discipline']) ?> </h1>
				</div>
			</section>

			<div class="container">
				<?php
				if (isset($table_circuits))
				{
					if ($table_circuits != null)
					{ ?>
						<form action="index.php?control=routing_page&action=results_loading_race" method="post">
							<label>Select Circuit : </label>
							<select name="circuit_selected">
									<?php
									foreach($table_circuits as $circuit)
									{ ?>
											<option value="<?php echo($circuit['idCircuit']); ?>"><?php echo($circuit['nameCircuit']); ?></option>
						<?php } ?>
							</select>
							<button type="submit" class="btn btn-primary">Submit</button>
						</form>
					<?php unset($table_circuits);
					}
					else
					{?>
		...
		<?php	}
				...
				if (isset($table_results))
				{
					...
					{ ?>
						<div class="container">
							<h2><?php echo($_SESSION['circuit_selected']);?> Grand Prix Result</h2>
							<table class="table">
								<thead>
									<tr>
										<th>Position</th>
								...
								<tbody>
									<?php foreach($table_results as $driver)
									{ ?>
										<tr>
											<th><?php echo($driver['position']) ?></th>
