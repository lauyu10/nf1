...
<html lang="en">
	<head>
		<title>NF1 Driver Information</title>
		<!-- Required meta tags -->
		<meta charset="utf-8">
	...
	<body>
		...
		<main role="main">
			...
			<section class="jumbotron text-center">
				<div class="container">
					<h1 class="jumbotron-heading">Driver Information : <?php echo($driver_info['namePilot']) ?> </h1>
				</div>
			</section>
				...
				<div class="card text-center">
  				<div class="card-header">
    				<ul class="nav nav-tabs card-header-tabs">
      				<li class="nav-item">
        				<a class="nav-link active" href="#">Biograpgy</a>
      				</li>
  					</ul>
					</div>
				  <div class="card-block">
				    <p class="card-text">Number : <?php echo($driver_info['pilotNumber']) ?></p>
						<p class="card-text">Date of Birth : <?php echo($driver_info['dateBirth']) ?></p>
						<p class="card-text">Place of Birth : <?php echo($driver_info['placeBirth']) ?></p>
						<p class="card-text">Team : <a href="index.php?control=routing_page&action=team_page&team=<?php echo($driver_info['idTeam']) ?>"><?php echo($driver_info['nameTeam']) ?></a></p>
				  </div>
				</div>
				<h1 class="jumbotron-heading"> Results of the Driver</h1>
				<table class="table">
					<thead>
						<tr>
							<th>Year of the season</th>
							<th>Team</th>
							<th>Race</th>
							<th>Place</th>
							<th>Position</th>
					...
					<tbody>
						<?php foreach($driver_race as $race)
						{ ?>
							<tr>
								<th><?php echo($race['year']) ?></th>
								<th><a href="index.php?control=routing_page&action=team_page&team=<?php echo($race['idTeam']) ?>"><?php echo($race['nameTeam']) ?></a></th>
								<th><?php echo($race['nameCircuit']) ?></th>
								<th><?php echo($race['place']) ?></th>
								<th><?php echo($race['position']) ?></th>
								<th><?php echo($race['points']) ?></th>
							</tr>
			<?php } ?>
					</tbody>
				</table>
